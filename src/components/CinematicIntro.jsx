...
const CinematicIntro = ({ onComplete }) => {
    const [phase, setPhase] = useState('WAITING'); // WAITING -> RADAR_TICKING -> DOSSIER -> CLASSIFIED -> WATER_TRANSITION -> BLUE_BACKGROUND -> SUBMARINE_PASS -> COMPLETE
    ...
    useEffect(() => {
        if (phase === 'DOSSIER') {
            ...
            // Phase 3 -> 4: Fade document
            setTimeout(() => {
                setPhase('FADE_DOCUMENT');
            }, 2500);
        } else if (phase === 'FADE_DOCUMENT') {
            // Phase 4 -> 5: Complete sequence, fade overlay out
            setTimeout(() => {
                setPhase('COMPLETE');
                setTimeout(() => onComplete(), 2000); // Tell parent to mount app fully after fade out
            }, 1000); // Wait for document to fade before fading black screen
        }
    }, [phase, onComplete]);
    ...
    return (
        <div style={{
            ...
            {/* SOLID BLACK OVERLAY THAT FADES TO REVEAL APP */}
            <div style={{
                position: 'absolute',
                inset: 0,
                backgroundColor: '#000',
                opacity: (phase === 'WAITING' || phase === 'RADAR_TICKING' || phase === 'BLACK_SCREEN' || phase === 'DOSSIER' || phase === 'CLASSIFIED' || phase === 'FADE_DOCUMENT') ? 1 : 0,
                transition: 'opacity 2s ease-out',
                zIndex: 1, // Underneath Intro content but covers app
                pointerEvents: 'none'
            }} />
            ...
            {/* Content Container (z-index higher than black overlay) */}
            <div style={{ position: 'absolute', inset: 0, zIndex: 5, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <div style={{
                        ...
                        transform: (phase === 'DOSSIER' || phase === 'CLASSIFIED') ? 'scale(1)' :
                            (phase === 'COMPLETE' || phase === 'FADE_DOCUMENT') ? 'scale(1.1)' : 'scale(0.9)',
